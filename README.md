...
While this extension really doesn't need the **Arduino IDE** installed - it only needs the ESP8266 package and tools - it's best to have it installed anyway.  This extension is meant to be a companion extension for the [Arduino for Visual Studio Code](https://github.com/Microsoft/vscode-arduino) plugin, as it relies on the **Arduino IDE** to compile and upload code through their toolchain.

## Features

* Works with or without the [Arduino for Visual Studio Code](https://github.com/Microsoft/vscode-arduino) plugin installed.  Just needs [**mkspiffs**](https://github.com/igrr/mkspiffs), **esptool**, and **espota.py** (if using OTA updating).
...
If you manually install the ESP8266 package, you can still use this extension by setting all the path location

## Getting Started

After installing this extension, you need to:

1. Create a new VSCode Project via the [Arduino for Visual Studio Code](https://github.com/Microsoft/vscode-arduino) extension (Command: `Arduino: Initialize`) or a new sketch with the **Arduino IDE** (`File/New`).

1. Add the URL **http://arduino.esp8266.com/stable/package_esp8266com_index.json** to the Additional Board URL settings.
...
1. Create and populate a directory with the files to be uploaded to the target **ESP8266 SPIFFS** partition - **`i.e. to replace the current contents!`**
...
7. Set the `path` setting to point the base directory of the files to be uploaded.

1. Set the `temp` setting to a filename that **mkspiffs** will create.
...
`ESP8266FS: Upload SPIFFS` - this command packs all of the files in the `dataFiles` subdirectory using the **mkspiffs** tool.  The `spiffsImage` will then be sent to the **ESP8266** using the **esptool** or **espota.py** tool (depending on the output port)

### Unpack

`ESP8266FS: Unpack SPIFFS` - this command will take the `spiffsImage` and unpack all the contents into the `dataFiles` folder using the **mkspiffs** tool.
###  List

`ESP8266FS: List SPIFFS` - this command will list the contents  of the `spiffsImage` using the **mkspiffs** tool.

### Visualize

`ESP8266FS: Visualize SPIFFS` - this command will "visualize" the contents  of the `spiffsImage` using the **mkspiffs** tool.
...
```json
{
    ...
    "esp8266fs.python": "C:/Python34/python.exe",
    ...
    "esp8266fs.espota.host.ip": "127.0.0.1",
...
* `arduino.path` - (*defined by the [Arduino for Visual Studio Code](https://github.com/Microsoft/vscode-arduino)*).   From their README: Path to Arduino, you can use a custom version of Arduino by modifying this setting to include the full path. Example: "`C:\\Program Files\\Arduino`" for Windows, "`/Applications`" for Mac, "`/home/$user/Downloads/arduino-1.8.1`" for Linux.

> Note: this parameter is **mandatory** for Linux as the `arduino` shell command can be placed anywhere and is not installed through the OS.  Do not use the **Arduino IDE** installable through a Ubuntu repositories - install the latest version found at [www.arduino.cc](https://www.arduino.cc/en/Main/Software) .
...
* `esp8266fs.python` - Path to **python** executable.  If not specified, then just "**python**" will be used.
...
* `esp8266fs.espota.host.ip` - IP address for the host.  Default is "**127.0.0.1**".
...
The following settings are per sketch settings (*defined by the [Arduino ESP8266 filesystem uploader](https://github.com/esp8266/arduino-esp8266fs-plugin)*).  You can find them in `.vscode/arduino.json` in the workspace.
...
Alternatively, if the `.vscode/arduino.json` file doesn't exist, or a particular setting is not defined, then the settings in the **Arduino IDE**'s `preferences.txt` file will be used instead.  This file is generated by the **Arduino IDE** and is set globally for ALL sketches.  The `.vscode/arduino.json` file is a "per sketch" setting.
